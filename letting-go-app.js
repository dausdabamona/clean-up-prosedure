...
// Uses ReleasingEngine (releasing-engine.js) for all guided releasing
        ...
        ReleasingEngine.startSequentialReleasing(wantingList, {
          ...
          onSequenceComplete: function(data) {
            ...
            setTimeout(function() {
              if (confirm('Kembali ke Manifesting Workbook?')) {
                localStorage.removeItem('lettingGo_returnContext');
                window.location.href = (returnContext && returnContext.returnUrl) || 'manifesting-workbook.html';
              }
            }, 500);
          }
        });
...
function showReturnBanner() {
  ...
  content.className = 'return-banner-content';

  var label = document.createElement('span');
  label.innerHTML = '🔗 Release dari: <strong>Manifesting Workbook</strong>';

  // Use textContent for user-supplied content to avoid HTML injection (XSS)
  var ctx = document.createElement('span');
  ctx.className = 'return-context';
  ctx.textContent = returnContext.content || '';
  ...
  btn.addEventListener('click', returnToManifesting);
